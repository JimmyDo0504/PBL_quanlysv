...
namespace quanlysv {
	...
	private:
		...
		void trangchusv::LoadDiemHocTap()
		{
			...
			// Lấy object DirectoryInfo của thư mục chứa .exe
			DirectoryInfo^ exeFolder = gcnew DirectoryInfo(Application::StartupPath);
			DirectoryInfo^ prFolder = exeFolder->Parent;        // => …\x64
			...
			// (Debug) Kiểm tra
			{
				...
				String^ dbg = String::Format(
					L"Lên 2 cấp: {0}\n=> Tìm thấy {1} file .dat",
					projectFolder, cnt);
				MessageBox::Show(dbg, L"Debug - Vị trí quét",
			...
			for each (String ^ filePath in files)
			{
				...
				/*{
					...
					String^ msg = String::Format(
						L"Đã đọc file môn: {0}\nSố SV: {1}", mon, mh.n);
					MessageBox::Show(msg, L"Debug - Đã đọc file",
						MessageBoxButtons::OK, MessageBoxIcon::Information);
				}*/
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->dataGridView1->Size = System::Drawing::Size(1362, 438);
			...
			this->Exit->Location = System::Drawing::Point(830, 21);
